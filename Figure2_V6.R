...
setwd ("F://Box Sync/BHASKAR UT AUSTIN/Research/WUE_20-21/Fresh/GWAS_WUE_v4/WUE_V4_Rfiles/Figure1/")
...
p1 <- Meandf %>%   
  ...
  geom_vline(xintercept = 0,linetype=2,size=0.25, color = "red")+
  geom_hline(yintercept = 0,linetype=2,size=0.25, color = "red")+
  #geom_abline(slope = 1,size=0.5,color="firebrick")+
...
###Correlation waterloss vs Biomass
...
plot1<-df3 %>% 
  ...
  # geom_hline(yintercept = 0.076, color = "blue", linetype = 2, size = 1)+
  # geom_vline(xintercept = 58.45857143, color = "blue", linetype = 2, size =1)+
...
plot1
#ggsave("WUEvsbiomass_V3.tiff", units="in", width=7, height=7, dpi=200, compression = 'lzw')


### Correlation delta v/WUE
...
##linear model (WUE_mean ~ mgDW_mean)
df3 <- read.csv("means_yogurt_V5.csv")
...
##linear model (WUE_mean ~ water_loss_mean)
df3 <- read.csv("means_yogurt_V5.csv")
...
plot_grid(p1, plot1,plot2,plot3,plot4, plot5, ncol = 3, nrow = 3)
...
ggsave("Fig2NEW.tiff", height = 9, width = 14, dpi = 300)

ggsave("Fig2NEW_A.svg", height = 9, width = 14, dpi = 300 )


##supplemental correlationships
...
###correlation of all comparisons
df3 <- read.csv("means_yogurt_V5.csv")
